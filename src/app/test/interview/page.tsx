...
export default function InterviewTestPage() {
  ...
  const handleStartInterview = async () => {
    setGenerating(true);
    try {
      const response = await fetch('/api/test/interview', {
        method: 'POST',
      ...
      <div className="min-h-screen bg-slate-900 p-8 flex items-center justify-center">
        <div className="w-full">
          ...
          <QuestionBasisSelection 
            title="Interview Test Configuration" 
            onSelect={(type) => {
              setBasis(type);
              if (type === 'ai') {
                setStep('ready');
      ...
      <div className="min-h-screen bg-slate-900 p-8 flex items-center justify-center">
        <div className="max-w-4xl w-full">
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-10 border border-white/20 text-center shadow-2xl">
            <div className="bg-amber-500/20 w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-amber-500/30">
              <BarChart3 className="h-10 w-10 text-amber-400" />
            </div>
            <h2 className="text-4xl font-bold text-white mb-4">Select Difficulty Level</h2>
            <p className="text-gray-400 mb-10">Target specific competence levels for the interview assessment.</p>
                ...
                >
                  <div className="flex flex-col items-start px-2">
                    ...
                    <span className="text-gray-500 text-sm">
                      {level === 'Any' ? 'Balanced mix of questions' : `Focus on ${level} complexity MCQs`}
                    </span>
                  </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-10 border border-white/20 shadow-2xl">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h2 className="text-3xl font-bold text-white">Interview Criteria</h2>
                <p className="text-gray-400">Select SOPs and question count for the manual interview set.</p>
              </div>
            </div>

            <div className="space-y-8">
              <div>
                <label className="block text-white font-semibold mb-4 text-lg">Select Target SOPs</label>
                {loadingBanks ? (
              ...
              <div className="pt-6 border-t border-white/10">
                ...
                >
                  {generating ? <Loader2 className="h-5 w-5 animate-spin" /> : <>Generate Interview Test <ChevronRight className="h-5 w-5" /></>}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="min-h-screen bg-slate-900 p-8 flex items-center justify-center">
        <div className="max-w-2xl w-full">
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-12 border border-white/20 text-center shadow-2xl relative overflow-hidden">
            ...
            <div className="bg-purple-500/20 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 border border-purple-500/30 animate-in zoom-in-50 duration-500">
              <BrainCircuit className="h-12 w-12 text-purple-400" />
            </div>
            
            <h2 className="text-4xl font-black text-white mb-4">AI Interview Ready</h2>
            <p className="text-gray-400 text-lg mb-10 leading-relaxed">
              Gemini will automatically compose a comprehensive set of <span className="text-purple-400 font-bold">20 questions</span> from across all available SOP banks.
            </p>
            
            <div className="grid grid-cols-2 gap-4 mb-10 text-left">
              <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                <div className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mb-1">Source</div>
                <div className="text-white font-semibold">ALL SOPs</div>
              </div>
              <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                <div className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mb-1">Scope</div>
                <div className="text-white font-semibold">Any Format</div>
              </div>
            </div>
            ...
            <button
              onClick={handleStartInterview}
              disabled={generating}
              className="w-full py-5 bg-gradient-to-r from-purple-600 via-pink-600 to-purple-700 text-white font-black text-xl rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-purple-500/20 flex items-center justify-center gap-3"
            >
              {generating ? (
                <>
  ...
  if (step === 'testing') {
    return (
      <TestRunner 
        questions={questions} 
        title={`Interview Test (${basis.toUpperCase()})`}
